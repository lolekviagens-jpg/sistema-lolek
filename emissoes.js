  ...
  async function analisarDocumento(prodId, tipo, imageSrc, zone, pernaAlvo) {
    ...
    try {
      ...
      if (tipo === "passagem") {
        ...
        fill("localizador", ex.localizador);
        fill("endereco", ex.endereco);
        fill("horario_checkin", ex.horario_checkin);
        fill("horario_checkout", ex.horario_checkout);
        const reembolsavelCfg = DADOS_CFG.hospedagem.find((f) => f.id === "reembolsavel");
        if (ex.reembolsavel && reembolsavelCfg.options.includes(ex.reembolsavel)) {
          fill("reembolsavel", ex.reembolsavel);
          // Dispara o "change" pra ligarCamposCondicionais revelar prazo/percentual agora
          // que "reembolsavel" veio preenchido pela IA (mesmo padrão do "tem_parada" do Trem).
          gel(`emi-prod-${prodId}-dados-reembolsavel`)?.dispatchEvent(new Event("change"));
        }
        fill("reembolso_prazo", ex.reembolso_prazo);
        fill("reembolso_percentual", ex.reembolso_percentual);
