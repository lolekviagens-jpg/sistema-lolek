...
(function () {
  ...
  async function analisarDocumento(prodId, tipo, imageSrc, zone) {
    ...
    const prompt = tipo === "passagem"
      ? `${contextoDataAtual()}\n\nAnalise este documento/print de passagem aérea com atenção a TODA a tabela de itinerário/voos, que pode ter mais de uma linha. Bilhetes oficiais de companhia aérea (LATAM, GOL, Azul etc.) costumam listar TODOS os voos da reserva numa única tabela "Itinerário", uma linha por trecho, MUITAS VEZES SEM escrever "IDA" ou "VOLTA" em lugar nenhum — a única forma de saber é contar as linhas da tabela. Se a tabela tiver 2 ou mais linhas de voo, é uma reserva de ida e volta (ou multi-trecho): NÃO ignore a segunda linha.\n\nRetorne SOMENTE um JSON válido, sem nenhum texto adicional:\n{\n  "trecho": "SIGLA_ORIGEM → SIGLA_DESTINO da 1ª linha/voo da tabela",\n  "localizador": "código/localizador da reserva, ou null",\n  "companhia": "nome da companhia aérea",\n  "voo": "número do voo da 1ª linha",\n  "horario_partida": "HH:MM da 1ª linha",\n  "horario_chegada": "HH:MM ou HH:MM (+1) da 1ª linha",\n  "conexoes": "Voo direto OU ex: 1 escala em GRU",\n  "milhas": número_inteiro_ou_null,\n  "taxa_embarque": valor_numerico_em_reais_ou_null,\n  "volta": {\n    "trecho": "SIGLA_ORIGEM → SIGLA_DESTINO da 2ª linha/voo da tabela (geralmente origem/destino invertidos em relação à 1ª)",\n    "localizador": "...", "companhia": "...", "voo": "número do voo da 2ª linha",\n    "horario_partida": "HH:MM da 2ª linha", "horario_chegada": "HH:MM ou HH:MM (+1) da 2ª linha",\n    "conexoes": "...", "milhas": número_inteiro_ou_null, "taxa_embarque": valor_numerico_em_reais_ou_null\n  } OU null — preencha "volta" com os dados da 2ª linha da tabela de voos SEMPRE que ela existir, mesmo que o documento não use as palavras "ida"/"volta". Só deixe "volta" null se houver de fato apenas UM voo no documento inteiro.\n}`
